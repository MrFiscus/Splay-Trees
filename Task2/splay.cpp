...
SplayTree::Node* SplayTree::semiSplay(Node* root, int key, int k)
{
    if (root == nullptr) return nullptr;
    if (k <= 0) return root;
    ...
    while (true) {
        ...
        if (key < root->key) {
            ...
            if (key < root->left->key && rotations < k) {
                root = rotateRight(root);
                rotations++;
                root->parent = nullptr;
                if (root->left == nullptr) break;
            ...
            if (key > root->right->key && rotations < k) {
                root = rotateLeft(root);
                rotations++;
                root->parent = nullptr;
                if (root->right == nullptr) break;
    ...
    // Reassemble
    leftTreeMax->right = root->left;
    if (leftTreeMax->right)
        leftTreeMax->right->parent = leftTreeMax;

    rightTreeMin->left = root->right;
    if (rightTreeMin->left)
        rightTreeMin->left->parent = rightTreeMin;
